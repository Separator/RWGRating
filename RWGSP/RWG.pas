...
// очистка переменной, хранящей данные по символам:
procedure RWGClearSymbols(var Symbol: TRWGSymbolsArr);
var
  i: Byte;
begin
...
// вытащить данные по символам из заданного файла:
function RWGGetSymbols(FileName: String): TRWGSymbolsArr;
var
...
// очистка переменной, хранящей данные по модиф. символам:
procedure RWGClearModifiedSymbols(var Symbols: TRWGModifiedSymbolsArr);
var
  i: Integer;
begin
...
// конвертирование простых символов в модифицированные:
procedure SymbolsToModified(var SymbolsArr: TRWGSymbolsArr; var ModifiedSymbols: TRWGModifiedSymbolsArr);
var
  i, j: Integer;
begin
...
begin
  ...
  FuncResult := false;
  while i <= len do
  // если есть совпадение - возвращаем совпавший символ:
  if (ModSymbols[i].Symbol <> '') and (ModSymbols[i].Symbol = Symbol) then
    begin
      FuncResult := true;
      GetModifSymbolBySymbol := ModSymbols[i];
    end;
  // если не найдено ни одного совпадения то возвращаем пустой символ:
  if (not FuncResult) then
    GetModifSymbolBySymbol.Symbol := '';
end;

// очистка переменной, хранящей данные по цветам:
procedure RWGClearColors(var RWGColorsArr: TRWGColorRecArr);
var
  i: Integer;
begin
...
// вытащить данные по цветам из заданного файла:
function RWGGetColors(FileName: String): TRWGColorRecArr;
var
...
// получить цвет по одной из его составляющих:
function GetColorByColorPart(ColorRecArr: TRWGColorRecArr; Color:TColor): TRWGColorRec;
var
  i, j: Integer;
  FuncResult: Boolean;
...
// --------------------------------------------------------------------------------------------
// собственно процедуры распознавания символов:

// очистка переменной, хранящей информацию о местонахождении строк с данными:
procedure RWGClearDataLinesCoords(var Lines: TRWGDataLineCoords);
var
  i: Integer;
begin
...
// вытаскивает координаты строки данных:
function CheckDataLineExist(Canvas: TCanvas; x,y: Integer; var RWGColor: TRWGColorRec; var LineCoords: TRWGDataLineCoord): Boolean;
const
  MAX_LINE_HEIGHT = 20;
  MIN_LINE_HEIGHT = 5;
...
begin
  ...
  //проверка ширины и высоты полученной строки данных:
...
// вытащить координаты строк с данными:
function GetDataLinesCoords(Canvas: TCanvas; var ColorRecArr: TRWGColorRecArr): TRWGDataLineCoords;
var
  x, y: Integer;
...
// очистка переменной, хранящей данные по строке:
...
// проверить "отдалённость" заданного символа:
...
// проверяет область на наличие символа определённого цвета:
procedure GetSymbol(Canvas: TCanvas; x,y: Integer; var Color: TRWGColorRec; var ModifiedSymbols: TRWGModifiedSymbolsArr; var GetSym: String; var ModSymbol: TRWGModifiedSymbol);
var
  i, len: Integer;
  FuncResult: Boolean;
  ...
  while (i <= len) and (not FuncResult) do
    begin
      if (ModifiedSymbols[i].Symbol <> '') then
        begin
          // проверка на отдалённость и соотв. маске:
...
begin
  ...
  for i := 1 to Length(DateLine.UserName) do
  begin
    if (DateLine.UserName[i] = 'a') then DateLine.UserName[i] := 'а';
    if (DateLine.UserName[i] = 'o') then DateLine.UserName[i] := 'о';
    if (DateLine.UserName[i] = 'p') then DateLine.UserName[i] := 'р';
    if (DateLine.UserName[i] = 'c') then DateLine.UserName[i] := 'с';
    if (DateLine.UserName[i] = 'B') then DateLine.UserName[i] := 'В';
    if (DateLine.UserName[i] = 'C') then DateLine.UserName[i] := 'С';
    if (DateLine.UserName[i] = 'X') then DateLine.UserName[i] := 'Х';
  end;
end; 
...
// вытаскиваем данные из заданной строки:
function GetDataLine(Canvas: TCanvas; var DataLineCoords: TRWGDataLineCoord; var ColorsArr: TRWGColorRecArr; var ModifiedSymbols, ReducedMS: TRWGModifiedSymbolsArr): TRWGDataLine;
const
  BEG_INDENT_X = 1;
  BEG_INDENT_Y = BEG_INDENT_X;
...
begin
  ...
  // создаем сокр. структуру для проверки только цифр и слеша:
  ...
  if (count > 0) then
    begin
      ...
      for i := 1 to Length(DataLineCoords) do
        if (DataLineCoords[i].ColorName <> '') then
          begin
            Label1.Caption := 'Разбирается строка №' + inttostr(j) + '.';
        ...
        Label1.Caption := 'Разбор закончен. Всего строк: ' + inttostr(j - 1) + '.';
    end;
end;

// получить координаты строки с временем:
...
begin
  ...
  TimeStr := ResStr;
  // собственно вытаскиваем время:
  minStr := ''; secStr := '';
  Delimiter := false;
  Delim := 'm'; // вот так вот записал)
  for i := 1 to Length(TimeStr) do
  begin
